...
class LogicMain(LogicModuleBase):
    ...
    def migration(self):
        try:
            ...
            if db_version < 2:
                ...
                cursor.execute(
                    f"SELECT * FROM {package_name}_setting WHERE key = 'interval'"
                )
                interval = cursor.fetchone()[2]
                cursor.execute(
                    f"UPDATE {package_name}_setting SET value = ? WHERE key = '{package_name}_interval'",
                    (interval,),
                )
                cursor.execute(
                    f"DELETE FROM {package_name}_setting WHERE key = 'interval'"
                )
                cursor.execute(
                    f"SELECT * FROM {package_name}_setting WHERE key = 'auto_start'"
                )
                auto_start = cursor.fetchone()[2]
                cursor.execute(
                    f"UPDATE {package_name}_setting SET value = ? WHERE key = '{package_name}_auto_start'",
                    (auto_start,),
                )
                cursor.execute(
                    f"DELETE FROM {package_name}_setting WHERE key = 'auto_start'"
                )
        ...
        try:
            for entity in ModelScheduler.get_list():
                ...
                if download["errorCode"] == 0:
                    ...
                    Thread(
                        target=LogicMain.download_check_function,
                        args=(video_url, download["index"], entity.key),
                    ).start()
                    entity.update()
                else:
    ...
    def get_recent_html() -> str:
        ...
        html = re.sub(
            r'onclick="vlive.tv.common.videoGa\(this\);"',
            r'onclick="link_click(this); return false;"',
            html,
        )
        html = re.sub(
